...
def get_size_path(path: Path) -> int:
    ...
    ## Parameters
    path: Path, the path item to be inspected

    ## Returns
    int: the size of the item at the given path, or 0 if no item exists.
    '''
    if not path.exists():
        return 0
    else:
...
def collect_runs(ws: Workspace, run_min: int, run_max: int) -> dict[int, int]:
    ...
    run_min: int, the first run
    run_max: int, the last run, inclusive

    ## Returns
    dict[int, int]: a dictionary where the keys are run numbers and the values are the size of the associated raw trace files. The
...
def create_run_stacks(config: Config, n_stacks: int) -> list[list[int]]:
    '''
    Create a set of runs to be processed for each stack in n_stacks. Each stack is
    intended to be handed off to a single processor. As such, the goal is to balance
    the load of work across all of the stacks. collect_runs is used to retrieve the list
    of runs sorted on their sizes. The list of stacks is then snaked, depositing one
    run in each stack in each iteration. This seems to provide a fairly balanced
    load without too much effort.

    ## Parameters
    config: Config, the project configuration
    n_stacks: int, the number of stacks, should be equal to number of processors

    ## Returns
    list[list[int]]: The stacks. Each stack is a list of ints, where each value
    is a run number for that stack to process.
    '''
