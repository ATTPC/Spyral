...
def phase_pointcloud_legacy(
    ...
    dv_df: pl.DataFrame = pl.read_csv(detector_params.drift_velocity_path)
    mm_tb: float = dv_df.row(by_predicate = (pl.col('run') == run), named = True)['micro_mean']
    w_tb: float = dv_df.row(by_predicate = (pl.col('run') == run), named = True)['wind_mean']
