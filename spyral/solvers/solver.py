...
SAMPLING_RANGE: np.ndarray = np.linspace(0.0, TIME_WINDOW, int(TIME_WINDOW / INTERVAL))
...
def get_sampling_steps(
    traj_data: np.ndarray, vertexX: float, vertexY: float, vertexZ: float
) -> np.ndarray:
...
def generate_trajectory(
    ...
    initial_value[3] = (
        speed
        * math.sin(fit_params["polar"].value)
        * math.cos(fit_params["azimuthal"].value)
    )
    initial_value[4] = (
        speed
        * math.sin(fit_params["polar"].value)
        * math.sin(fit_params["azimuthal"].value)
    )
    ...
    result = integrate.solve_ivp(
        ...
        initial_value,
        method="BDF",
        args=(Bfield, Efield, target, ejectile),
        t_eval=SAMPLING_RANGE,
        jac=jacobian,
    )
...
def create_params(
    initial_value: Guess, ejectile: NucleusData, data: np.ndarray
) -> Parameters:
    ...
    min_x = (
        ...
        - initial_value.vertex_x * 0.001 * uncertainty_position * 2.0
    )
    max_x = (
        initial_value.vertex_x * 0.001
        + initial_value.vertex_x * 0.001 * uncertainty_position * 2.0
    ...
    min_y = (
        ...
        - initial_value.vertex_y * 0.001 * uncertainty_position * 2.0
    )
    max_y = (
        initial_value.vertex_y * 0.001
        + initial_value.vertex_y * 0.001 * uncertainty_position * 2.0
    ...
    min_z = (
        ...
        - initial_value.vertex_z * 0.001 * uncertainty_position * 2.0
    )
    max_z = (
        initial_value.vertex_z * 0.001
        + initial_value.vertex_z * 0.001 * uncertainty_position * 2.0
    ...
    fit_params.add(
        "vertex_x", initial_value.vertex_x * 0.001, min=min_x, max=max_x, vary=True
    )
    fit_params.add(
        "vertex_y", initial_value.vertex_y * 0.001, min=min_y, max=max_y, vary=True
    )
    fit_params.add(
        "vertex_z", initial_value.vertex_z * 0.001, min=min_z, max=max_z, vary=True
    )
    return fit_params
...
def fit_model(
    ...
    result: MinimizerResult = minimize(
        objective_function,
        fit_params,
        args=(traj_data, bfield, efield, target, ejectile, initial_value.direction),
    )
