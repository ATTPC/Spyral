...
def phase_4(
    ...
    pid: ParticleID = load_particle_id(
        ws.get_gate_file_path(solver_params.particle_id_filename), nuclear_data
    )
    if pid is None:
    ...
    print(
        f"Running physics solver on clusters in {cluster_path} using initial guesses from {estimate_path}"
    )
    print(
        f"Selecting data which corresponds to particle group from {solver_params.particle_id_path}"
    )

    # Select the particle group data, convert to dictionary for row-wise operations
    estimates_gated = (
        estimate_df.filter(pl.struct(["dEdx", "brho"]).map(pid.cut.is_cols_inside))
        .collect()
        .to_dict()
    )
    ...
    for row, event in enumerate(estimates_gated["event"]):
        if count > flush_val:
            ...
            flush_count += 1
            print(
                f"\rPercent of data processed: {int(flush_count * flush_percent * 100)}%",
                end="",
            )
        count += 1
        ...
        # Do the solver
        iv = Guess(
            polar=estimates_gated["polar"][row],
            azimuthal=estimates_gated["azimuthal"][row],
            brho=estimates_gated["brho"][row],
            vertex_x=estimates_gated["vertex_x"][row] * 0.001,
            vertex_y=estimates_gated["vertex_y"][row] * 0.001,
            vertex_z=estimates_gated["vertex_z"][row] * 0.001,
            direction=Direction(estimates_gated["direction"][row]),
        )
