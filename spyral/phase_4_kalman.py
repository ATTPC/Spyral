...
def phase_4_kalman(
    ...
    print(
        f"Selecting data which corresponds to particle group from {solver_params.particle_id_filename}"
    )
    ...
    for row, event in enumerate(estimates_gated["event"]):
        ...
        solve_physics_kalman(
            cidx, cluster, guess, detector_params, target, pid.nucleus, results
        )
