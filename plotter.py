...
def plot(run_min: int, run_max: int, ws: Workspace):
...
def draw_gate(run_min: int, run_max: int, ws: Workspace):
    ...
    grammer.add_hist2d('pid', (200, 150), ((0.0, 20000.0), (0.0, 1.5)))
    for run in range(run_min, run_max+1):
        run_path = ws.get_estimate_file_path_parquet(run)
        if not run_path.exists():
            continue
    ...
    mesh = grammer.draw_hist2d('pid', ax)
...
def main_plot(config: Config):
    ws = Workspace(config.workspace)
    plot(config.run.run_min, config.run.run_max, ws, config.solver.particle_id_path)
