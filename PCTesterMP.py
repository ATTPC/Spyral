...
if __name__ == '__main__':
	...
	cores = cpu_count()
	print('Using ', cores, ' cores for deconvolution.')
	...
	for trace_num in range(len(all_traces)):
    		...
    		if len(peaks) != 0:
        		...
        		for peak in peaks:
            			if (((peak+num_pts) < len(response)) and ((peak-num_pts) > 0)):
	...
	#print('Time it takes to filter the point cloud: ', time.time()-start)

	interact = False

	if (interact == True):
    		fig = go.Figure(data=[go.Scatter3d(x=pc[:,0], y=pc[:,1], z=pc[:,2],
                                       mode='markers', marker = dict(size = 2, color = pc[:,3], colorscale = 'viridis'))])
		...
		#plt.scatter(padxy['x'], padxy['y'], c = pad_loc.sort_values(by=['pad'])['size'], alpha = 0.25)
		plt.scatter(pc[:,0], pc[:,1], c = pc[:,3], cmap = 'seismic', s = 2.5, alpha = 1)
		plt.xlim(-292, 292)
		plt.ylim(-292, 292)
		plt.grid()
		plt.show()
