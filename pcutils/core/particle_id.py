...
@dataclass
class ParticleID:
    cut: Cut2D = field(default_factory=Cut2D)
    nucleus: NucleusData = field(default_factory=NucleusData)

def load_particle_id(cut_path: Path, nuclear_map: NuclearDataMap) -> Optional[ParticleID]:
    json_data = load(cut_path)
    pid = ParticleID()

    if 'name' not in json_data or 'verticies' not in json_data or 'Z' not in json_data or 'A' not in json_data:
        print(f'ParticleID could not load cut in {cut_path}, the requested data is not present.')
        return None
    
    pid.cut = Cut2D(json_data['name'], json_data['verticies'])
    pid.nucleus = nuclear_map.get_data(json_data['Z'], json_data['A'])
